...
citra1 = cv2.imread("C:/Users/Aldi Triwijaya/Documents/kuliah/semester 3/pengolahan citra digital praktik/Gambar/chelsea - rgb.jpg")
citra2 = cv2.imread("C:/Users/Aldi Triwijaya/Documents/kuliah/semester 3/pengolahan citra digital praktik/Gambar/kacamata.jpg")
...
# citra_gray1 = np.zeros((jum_baris, jum_kolom))        # Gunakan ini jika ingin menggunakan citra grayscale
# citra_gray2 = np.zeros((jum_baris, jum_kolom))        # Gunakan ini jika ingin menggunakan citra grayscale
citra_and = np.zeros((jum_baris, jum_kolom, 3))         # Gunakan ini jika ingin menggunakan citra RGB
...
for i in range(jum_baris):
    for j in range(jum_kolom):
        ...
        citra_and[i, j, 0] = max(b1[i, j], b2[i, j])
        citra_and[i, j, 1] = max(g1[i, j], g2[i, j])
        citra_and[i, j, 2] = max(r1[i, j], r2[i, j])
